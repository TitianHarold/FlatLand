...
const motionCovers=import.meta.glob('../stories/*/assets/cover-motion.json',{eager:true,import:'default'});
...
export const stories=Object.entries(documents).flatMap(([path,story])=>{
  ...
  return [{id,title:story.title,description:story.description??'',source:sources[path],settings:settings[path.replace('/story.json','/settings.json')],cover:covers[id],motionCover:motionCovers[path.replace('/story.json','/assets/cover-motion.json')]}];
});
